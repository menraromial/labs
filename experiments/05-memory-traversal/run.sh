...
while IFS=, read -r run_id core cpu seed rounds warmup_rounds min_log2 max_log2 accesses_log2; do
    base=$(printf '%s/run-%03d' "$directory" "$run_id")
    taskset -c "$cpu" ./build/traversal --run-id "$run_id" --core "$core" --seed "$seed" \
        --rounds "$rounds" --warmup-rounds "$warmup_rounds" --min-log2 "$min_log2" \
        --max-log2 "$max_log2" --accesses-log2 "$accesses_log2" \
